...
export class CodeReviewProvider {
    ...
    private async reviewFile(filePath: string, content?: string): Promise<void> {
        await this.notificationManager.showProgress(
            ...
            async (progress) => {
                ...
                try {
                    console.log('🚀 Starting code review with request:', request);
                    const reviewId = await this.mcpAPI.startCodeReview(request);
                    console.log('📋 Received review ID:', reviewId, 'Type:', typeof reviewId);
    ...
    private async pollForResults(
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            try {
                console.log(`🔍 Polling for results with reviewId: ${reviewId} (attempt ${attempt + 1})`);
